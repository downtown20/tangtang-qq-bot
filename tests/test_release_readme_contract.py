...
def test_release_tag_is_three_part():
    """版本号必须三段式 `vX.YY.ZZ`（2026-09-20 主人定，当天改过一次口径）。

    两位式（v1.6 / v2.0）弃用——那样很快就数到 2.x，而糖糖还在早期。

    ⚠ 主号**不能是 0**：先试过 `v0.01.00`，结果它在 Release 列表里排到最底下
    （0.x 永远小于 1.x），主人当天就指出来了。现在主号沿用 1，
    中号两位递增：`v1.06.00` → `v1.07.00` → …（还有 93 个位子才到 2.0）。
    """
    tag = _packer_tag()
    assert re.fullmatch(r"v\d+\.\d{2}\.\d{2}", tag), (
        f"版本号格式不对：{tag!r}——应为三段式 vX.YY.ZZ（如 v1.06.00）。\n"
        f"  两位式（v1.6 / v2.0）已弃用，见 tools/打包发布版本.py 里 TAG 上方的规则")
    major = int(tag[1:].split(".")[0])
    assert major >= 1, (
        f"主号是 0（{tag!r}）——0.x 在 Release 列表里会排到所有 1.x 底下。\n"
        f"  上一版是 v1.05.00，所以下一版应当 ≥ v1.06.00")
