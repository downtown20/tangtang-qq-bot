...
def check_resources():
    ...
    if spk.exists():
        print(f"  ℹ GPT-SoVITS 音色目录: {len(list(spk.iterdir()))} 个条目")


def check_ports():
    _sec("4️⃣ 外部服务端口（运行时检查）")
    print("  ℹ 糖糖未运行时端口关闭是正常的——main.py 启动会自动拉起语音服务")
    print("  ℹ 本机若配置为不使用本地语音/本地识图（如笔记本），语音与 Ollama 端口项可忽略")
...
def main():
    ...
    print(f"\n{'=' * 58}")
    print("💡 在两台机器各跑一遍，逐行对比。❌ 即缺失功能，⚠ 即需要留意。")
